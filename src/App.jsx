...
const MEALS = [
  { id: "breakfast", label: "Breakfast", emoji: "\u{1F305}", time: "7:00am", kcal: 362, protein: 50, carbs: 24, fat: 7,
    items: [
  ...
  { id: "dinner", label: "Dinner", emoji: "\u{1F319}", time: "6:00pm", kcal: 831, protein: 57, carbs: 72, fat: 47,
    items: [
      ...
      { name: "Everest Paneer 100g", kcal: 347, protein: 20.9, carbs: 4.1, fat: 27.7 },
...
const FOOD_DB = [
  ...
  { name: "Crispy Bhindi 30g (air fried)", kcal: 40, protein: 1.2, carbs: 4.2, fat: 1.5 },
...
const SUPPLEMENTS = [
  ...
  { id: "iq", label: "IQ Supplement", time: "Morning", emoji: "\u{1F9E0}", color: "#3b82f6" },
  { id: "ashwa", label: "Ashwagandha KSM-66", time: "With Dinner", emoji: "\u{1F331}", color: "#22c55e" },
  { id: "mag", label: "Magnesium Glycinate", time: "Before Bed", emoji: "\u{1F634}", color: "#a855f7" },
];

const PLAN_DETAIL = [
  { id: "b", label: "Breakfast", emoji: "\u{1F305}", time: "7:00am", kcal: 362, protein: 50, carbs: 24, fat: 7,
  ...
  { id: "d", label: "Dinner", emoji: "\u{1F319}", time: "6:00pm", kcal: 831, protein: 57, carbs: 72, fat: 47,
    prep: "Grill paneer until golden. Assemble rotlis with veg + sauces. Weigh nasto + peanuts. Shake whey.",
    ingredients: ["2 homemade rotlis (Aashirvaad atta ~25g each)", "100g Paneer (grilled)", "Mixed salad veg", "Green chutney + sweet chilli sauce", "30g Nasto (weighed)", "30g KP Roasted Peanuts (weighed)", "25g Myprotein Essential Whey"] },
];
...
const CARDIO_TYPES = ["Treadmill", "Incline Walk", "Cross Trainer", "Cycling", "Rowing", "Running (outdoor)", "HIIT", "Swimming"];
      ...
      <div style={{ padding:"14px 14px 0" }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:12 }}>
          ...
          <Card style={{ margin:0, textAlign:"center", padding:"12px 8px" }}>
            ...
            {change&&<div style={{ fontSize:9, color:parseFloat(change)<=0?"#22c55e":"#e85d26" }}>{parseFloat(change)<=0?"down":"up"} {Math.abs(change)}kg</div>}
          </Card>
          <Card style={{ margin:0, textAlign:"center", padding:"12px 8px" }}>
          ...
          <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
            {SUPPLEMENTS.map(s=>(
              <div key={s.id} style={{ padding:"5px 12px", borderRadius:99, background:suppLog.includes(s.id)?s.color+"20":"#f5f5f5", border:`1.5px solid ${suppLog.includes(s.id)?s.color+"40":"#eee"}`, fontSize:11, color:suppLog.includes(s.id)?s.color:"#bbb", fontWeight:600 }}>
                {suppLog.includes(s.id)?"check ":""}{s.label.split(" ")[0]}
              </div>
            ))}
          </div>
        </Card>
...
function LogTab({ foodLog, totals, onAdd, onRemove, myFoods, onSaveFood, onDeleteMyFood, selectedDate, onDateChange }) {
  ...
  function goForward() {
    ...
    if (next <= getToday()) onDateChange(next);
  }

  const tabs = [["quick","Meals"],["myfoods","My Foods ("+myFoods.length+")"],["search","Search"],["manual","Manual"],["log","Log ("+foodLog.length+")"]];
  ...
  return (
    <div>
      ...
      <div style={{ background:"#fff", padding:"10px 14px", display:"flex", alignItems:"center", gap:10, borderBottom:"1px solid #eee" }}>
        <button type="button" onClick={goBack} style={{ width:36, height:36, borderRadius:10, border:"1.5px solid #eee", background:"#f8f6f2", cursor:"pointer", fontSize:18, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center" }}>{"<"}</button>
        <input type="date" value={selectedDate} max={getToday()} onChange={e=>e.target.value&&onDateChange(e.target.value)}
        ...
        <button type="button" onClick={goForward} disabled={isToday} style={{ width:36, height:36, borderRadius:10, border:"1.5px solid #eee", background:isToday?"#f0f0f0":"#f8f6f2", cursor:isToday?"not-allowed":"pointer", fontSize:18, fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", color:isToday?"#ccc":"#333" }}>{">"}</button>
        {!isToday&&<button type="button" onClick={()=>onDateChange(getToday())} style={{ padding:"6px 12px", borderRadius:10, background:"#1a1a2e", color:"#fff", border:"none", cursor:"pointer", fontSize:11, fontWeight:700, whiteSpace:"nowrap" }}>Today</button>}
      </div>
      {!isToday&&(
        <div style={{ background:"#fff7ed", padding:"8px 14px", fontSize:12, color:"#e85d26", fontWeight:600, textAlign:"center" }}>
          Editing: {fmtDateLong(selectedDate)} - changes save to this date
        </div>
      )}
      ...
      <div style={{ padding:"14px 14px 0" }}>
                    ...
                    <div onClick={()=>setPlanOpen(planOpen===meal.id?null:meal.id)} style={{ cursor:"pointer" }}>
                      ...
                      {planOpen===meal.id&&(
                        <div style={{ marginTop:10 }}>
                          ...
                          <div style={{ background:"#f8f6f2", borderRadius:10, padding:"8px 12px", marginTop:8, fontSize:11, color:"#666" }}>Prep: {meal.prep}</div>
                        </div>
                      )}
                    </div>
                  </Card>
            ...
            {myFoods.length===0?(
              <Card>
                <div style={{ textAlign:"center", padding:"30px 0" }}>
                  <div style={{ fontSize:40, marginBottom:10 }}>star</div>
                  <div style={{ fontSize:14, fontWeight:600, color:"#888", marginBottom:6 }}>No saved foods yet</div>
                  <div style={{ fontSize:12, color:"#aaa" }}>Add foods in Manual tab then tap Save to My Foods</div>
                </div>
              </Card>
            ):(
              <div>
                <div style={{ fontSize:12, color:"#888", marginBottom:12 }}>Tap + to add to log</div>
                {myFoods.map((food,i)=>(
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:10, background:"#fff", borderRadius:14, padding:"12px 14px", marginBottom:8, boxShadow:"0 2px 8px rgba(0,0,0,0.05)", border:"1px solid #f0eee8" }}>
                    ...
                    <button type="button" onClick={()=>onDeleteMyFood(food.name)} style={{ width:32, height:32, borderRadius:"50%", background:"#fff0f0", color:"#ef4444", border:"none", cursor:"pointer", fontSize:14 }}>x</button>
        ...
        {subTab==="search"&&(
          <Card>
            ...
            <div style={{ fontSize:11, color:"#888", marginBottom:14 }}>Search from {FOOD_DB.length} common UK and Indian foods.</div>
        ...
        {subTab==="manual"&&(
          <Card>
            <div style={{ fontSize:13, fontWeight:700, color:"#1a1a2e", marginBottom:4 }}>Manual Entry</div>
            ...
            {manual.name&&manual.kcal&&(
              <button type="button" onClick={()=>{ onSaveFood({name:manual.name,kcal:Number(manual.kcal)||0,protein:Number(manual.protein)||0,carbs:Number(manual.carbs)||0,fat:Number(manual.fat)||0}); showToast("Saved: "+manual.name+" to My Foods"); }} style={{ width:"100%", padding:"12px", background:"#f59e0b", color:"#fff", border:"none", borderRadius:13, fontSize:13, fontWeight:700, cursor:"pointer" }}>Save to My Foods</button>
            )}
          </Card>
        )}
          ...
          <div>
            {foodLog.length===0?(
              <div style={{ textAlign:"center", padding:"50px 20px", color:"#bbb" }}>
                <div style={{ fontSize:48, marginBottom:14 }}>clipboard</div>
                <div style={{ fontSize:14, fontWeight:600, color:"#888" }}>Nothing logged for {isToday?"today":fmtDateLong(selectedDate)}</div>
                <div style={{ fontSize:12, color:"#aaa", marginTop:6 }}>Use the other tabs to add food</div>
              </div>
            ):(
              <>
                <Card style={{ background:"#1a1a2e", border:"none" }}>
                  <div style={{ fontSize:10, color:"#8899bb", fontFamily:"monospace", marginBottom:6 }}>TOTAL FOR {isToday?"TODAY":fmtDateLong(selectedDate).toUpperCase()}</div>
                ...
                {foodLog.map(item=>(
                  <Card key={item.id}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      ...
                      <button type="button" onClick={()=>onRemove(item.id)} style={{ width:34, height:34, borderRadius:"50%", background:"#fff0f0", color:"#ef4444", border:"none", cursor:"pointer", fontSize:15, flexShrink:0, marginLeft:10 }}>x</button>
                    </div>
                  </Card>
                ))}
              </>
...
function TrainTab({ sessions, onSaveSession, onDeleteSession, weeklyData }) {
  ...
  function addSet(exIdx) { setExercises(prev=>prev.map((e,i)=>i===exIdx?{...e,sets:[...e.sets,{reps:"",weight:""}]}:e)); }
  function removeSet(exIdx,setIdx) { setExercises(prev=>prev.map((e,i)=>i===exIdx?{...e,sets:e.sets.filter((_,si)=>si!==setIdx)}:e)); }
  function updateSet(exIdx,setIdx,field,val) { setExercises(prev=>prev.map((e,i)=>i===exIdx?{...e,sets:e.sets.map((s,si)=>si===setIdx?{...s,[field]:val}:s)}:e)); }
  function removeExercise(exIdx) { setExercises(prev=>prev.filter((_,i)=>i!==exIdx)); }
  ...
  async function saveSession() {
    ...
    const session = { id, date:today, mode, sessionName:sessionName||(mode==="cardio"?cardioData.type:"Session"), exercises:mode!=="cardio"?exercises:[], cardioData:mode==="cardio"?cardioData:null, totalVolume, savedAt:new Date().toISOString() };
  ...
  function getProgressData(exName) {
    return sessions.filter(s=>s.exercises&&s.exercises.find(e=>e.name===exName)).map(s=>{ const ex=s.exercises.find(e=>e.name===exName); const topWeight=Math.max(...ex.sets.map(st=>Number(st.weight)||0)); const topReps=ex.sets.find(st=>Number(st.weight)===topWeight)?.reps||0; return { date:fmtDate(s.date), weight:topWeight, reps:Number(topReps) }; }).slice(-12);
  }
  ...
  return (
    <div>
      <Toast msg={toast}/>
      <DarkHeader tag="TRAINING" title="Train">
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, background:"rgba(255,255,255,0.06)", borderRadius:14, padding:12 }}>
          <div style={{ textAlign:"center" }}>
            <div style={{ fontSize:18, fontWeight:800, color:"#e85d26" }}>{sessions.filter(s=>s.date===today).length>0?"done":"-"}</div>
      ...
      <div style={{ padding:"14px 14px 0" }}>
            ...
            {!mode?(
              <Card>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10 }}>
                  {[["Weights","weights","Gym - sets, reps, kg"],["Bodyweight","bodyweight","No equipment"],["Cardio","cardio","Run, cycle, row"]].map(([label,val,sub])=>(
              ...
              <div>
                <Card>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                    <div style={{ fontSize:14, fontWeight:700 }}>{mode==="weights"?"Weights":mode==="bodyweight"?"Bodyweight":"Cardio"}</div>
                  ...
                  <input value={sessionName} onChange={e=>setSessionName(e.target.value)} placeholder={mode==="cardio"?"Session name (optional)":"e.g. Chest and Triceps"}
                          ...
                          <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                            {exList.map(ex=>(
                              <button key={ex} type="button" onClick={()=>addExercise(ex)} disabled={!!exercises.find(e=>e.name===ex)} style={{ padding:"6px 12px", borderRadius:99, border:"1.5px solid #eee", background:exercises.find(e=>e.name===ex)?"#e5e5e5":"#f8f6f2", color:exercises.find(e=>e.name===ex)?"#aaa":"#333", fontSize:12, fontWeight:600, cursor:exercises.find(e=>e.name===ex)?"default":"pointer" }}>
                                {ex} {exercises.find(e=>e.name===ex)?"done":"+"}
                              </button>
                            ))}
                          </div>
                        </div>
                    ...
                    {exercises.map((ex,exIdx)=>(
                      <Card key={exIdx}>
                        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                          ...
                          <button type="button" onClick={()=>removeExercise(exIdx)} style={{ width:28, height:28, borderRadius:"50%", background:"#fff0f0", color:"#ef4444", border:"none", cursor:"pointer", fontSize:13 }}>x</button>
                        </div>
                        ...
                        {ex.sets.map((set,setIdx)=>(
                          <div key={setIdx} style={{ display:"grid", gridTemplateColumns:mode==="weights"?"40px 1fr 1fr 40px":"40px 1fr 40px", gap:6, marginBottom:6 }}>
                            ...
                            <button type="button" onClick={()=>removeSet(exIdx,setIdx)} style={{ width:36, height:40, borderRadius:10, background:"#fff0f0", color:"#ef4444", border:"none", cursor:"pointer", fontSize:14 }}>x</button>
                          </div>
                        ))}
          ...
          <div>
            {sessions.length===0?(
              <div style={{ textAlign:"center", padding:"50px 20px" }}>
                <div style={{ fontSize:48, marginBottom:14 }}>gym</div>
                  ...
                  <div onClick={()=>setExpandedSession(expandedSession===session.id?null:session.id)} style={{ cursor:"pointer" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                      <div>
                        ...
                        <div style={{ fontSize:15, fontWeight:700 }}>{session.sessionName}</div>
                      ...
                      <button type="button" onClick={e=>{ e.stopPropagation(); onDeleteSession(session.id); }} style={{ width:30, height:30, borderRadius:"50%", background:"#fff0f0", color:"#ef4444", border:"none", cursor:"pointer", fontSize:13 }}>x</button>
                    </div>
                  </div>
                      ...
                      {session.mode==="cardio"?(
                        ...
                        session.exercises?.map((ex,i)=>(
                          <div key={i} style={{ marginBottom:10, background:"#f8f6f2", borderRadius:10, padding:10 }}>
                            ...
                            {ex.sets.map((set,si)=>(
                              <div key={si} style={{ fontSize:12, color:"#555", marginBottom:2 }}>Set {si+1}: {set.reps} reps{set.weight?" @ "+set.weight+"kg":""}</div>
                            ))}
                          </div>
                        ))
                      )}
        ...
        {subTab==="progress"&&(
          <div>
            {allExNames.length===0?(
              <div style={{ textAlign:"center", padding:"50px 20px" }}>
                <div style={{ fontSize:48, marginBottom:14 }}>chart</div>
                ...
                <Card>
                  ...
                  {allExNames.map(exName=>{ const pb=getPB(exName); if(!pb||!pb.weight) return null; return (
                    <div key={exName} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #f8f8f8" }}>
                      ...
                      <span style={{ fontSize:13, fontWeight:700, color:"#e85d26" }}>{pb.weight}kg x {pb.reps} reps</span>
                    </div>
                  );})}
                </Card>
                <Card>
        ...
        {subTab==="analysis"&&<WeeklyAnalysis sessions={sessions} weeklyData={weeklyData}/>}
      </div>
    </div>
  );
}
...
function WeeklyAnalysis({ sessions, weeklyData }) {
  ...
  function buildReport() {
    ...
    const avgKcal = weeklyData.filter(d=>d.kcal>0).length>0?Math.round(weeklyData.reduce((s,d)=>s+d.kcal,0)/weeklyData.filter(d=>d.kcal>0).length):0;
    const avgProtein = weeklyData.filter(d=>d.protein>0).length>0?Math.round(weeklyData.reduce((s,d)=>s+d.protein,0)/weeklyData.filter(d=>d.protein>0).length):0;
    ...
    r += "ABOUT PARTH:\n";
    r += "Age 28, 168cm, starting weight 67kg, UK dentist\n";
    r += "Goal: Body recomposition - reduce body fat from 22.7% to 12-14% (visible abs)\n";
    r += "Diet: Vegetarian, no eggs\n";
    r += "Calorie target: 1950 kcal | Protein target: 134g\n\n";
    r += "--- NUTRITION ---\n";
    r += "Avg daily calories: "+avgKcal+" kcal (target: 1950)\n";
    r += "Avg daily protein: "+avgProtein+"g (target: 134g)\n";
    r += "Daily breakdown:\n";
    weeklyData.forEach(d=>{ r += "  "+d.shortLabel+": "+d.kcal+" kcal, "+d.protein+"g protein\n"; });
    r += "\n--- BODY ---\n";
    if (weights.length>0) { r += "Latest weight: "+weights[weights.length-1]+"kg\n"; if(weightChange) r += "Weight change this week: "+weightChange+"kg\n"; }
    else r += "No weight logged this week\n";
    r += "\n--- TRAINING ("+weekSessions.length+" sessions) ---\n";
    if (weekSessions.length===0) { r += "No training sessions this week\n"; }
    else {
      weekSessions.forEach(s=>{
        r += "\n"+fmtDate(s.date)+" - "+s.sessionName+" ("+s.mode+")\n";
        if (s.mode==="cardio") { r += "  "+s.cardioData?.type+": "+s.cardioData?.duration+" mins"; if(s.cardioData?.distance) r += ", "+s.cardioData.distance+"km"; r += "\n"; }
  ...
  return (
    <div>
      <Card>
        ...
        <div style={{ fontSize:12, color:"#666", lineHeight:1.7, marginBottom:16 }}>Compiles your full week into a report. Copy and paste to Claude for detailed analysis.</div>
        <div style={{ background:"#f8f6f2", borderRadius:12, padding:14, marginBottom:14 }}>
          <div style={{ fontSize:11, color:"#555", fontFamily:"monospace", whiteSpace:"pre-wrap", lineHeight:1.6, maxHeight:200, overflow:"auto" }}>{buildReport()}</div>
        </div>
      ...
      <Card style={{ background:"#f0fdf4", border:"1px solid #86efac" }}>
        <div style={{ fontSize:12, fontWeight:700, color:"#22c55e", marginBottom:8 }}>How to use</div>
        <div style={{ fontSize:11, color:"#555", lineHeight:1.7 }}>1. Tap Copy Report{" "}2. Open Claude{" "}3. Paste the report{" "}4. Get detailed analysis and recommendations</div>
      </Card>
    </div>
  );
}
...
function BodyTab({ weightLog, onAdd, photos, onAddPhoto, onDeletePhoto }) {
  ...
  return (
    <div>
      <DarkHeader tag="BODY STATS" title="Body Tracker">
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, background:"rgba(255,255,255,0.06)", borderRadius:14, padding:12 }}>
      ...
      <SubTabs tabs={[["weight","Weight"],["photos","Photos ("+photos.length+")"]]} active={bodyTab} onChange={setBodyTab}/>

      <div style={{ padding:"14px 14px 0" }}>
        {bodyTab==="weight"&&(
          <div>
            <Card>
              <div style={{ fontSize:13, fontWeight:700, color:"#1a1a2e", marginBottom:12 }}>{todayEntry?"Today: "+todayEntry.weight+"kg":"Log Today Weight"}</div>
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button type="button" onClick={handleSave} style={{ padding:"14px 20px", background:"#1a1a2e", color:"#fff", borderRadius:13, border:"none", cursor:"pointer", fontSize:14, fontWeight:700, minWidth:64, minHeight:50 }}>{saved?"Saved":"Save"}</button>
              </div>
          ...
          <div>
            <Card>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
                {[["Take Photo","environment"],["Upload Photo",""]].map(([label,capture])=>(
                  <div key={label} style={{ position:"relative", borderRadius:14, overflow:"hidden" }}>
                    <div style={{ padding:"16px 10px", background:label==="Take Photo"?"#1a1a2e":"#f8f6f2", color:label==="Take Photo"?"#fff":"#1a1a2e", textAlign:"center", fontSize:13, fontWeight:700, border:label!=="Take Photo"?"1.5px solid #eee":"none", pointerEvents:"none" }}>
      ...
      <DarkHeader tag="DAILY STACK" title="Supplements">
        <div style={{ background:"rgba(255,255,255,0.06)", borderRadius:14, padding:14 }}>
          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
            <span style={{ fontSize:12, color:"#8899bb" }}>Today</span>
          ...
          <Card key={group.title}>
            ...
            {group.items.map(supp=>{ const done=suppLog.includes(supp.id); return (
              <button key={supp.id} type="button" onClick={()=>onToggle(supp.id)} style={{ width:"100%", display:"flex", alignItems:"center", gap:14, padding:"14px", marginBottom:8, borderRadius:13, cursor:"pointer", textAlign:"left", background:done?supp.color+"15":"#f8f6f2", border:`1.5px solid ${done?supp.color+"50":"#f0ece8"}` }}>
                <div style={{ width:36, height:36, borderRadius:"50%", flexShrink:0, background:done?supp.color:"#e5e5e5", display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, color:done?"#fff":"#aaa", fontWeight:700 }}>{done?"done":supp.emoji}</div>
                <div style={{ flex:1 }}>
                ...
                {done&&<div style={{ fontSize:18 }}>check</div>}
              </button>
            );})}
          </Card>
        ))}
...
export default function App() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      for (let i=6;i>=0;i--) {
        ...
        days.push({ date:dateStr, shortLabel:dayLabel(dateStr), kcal:Math.round(df.reduce((s,f)=>s+(Number(f.kcal)||0),0)), protein:Math.round(df.reduce((s,f)=>s+(Number(f.protein)||0),0)), weight:wEntry?wEntry.weight:null });
      }
      setWeeklyData(days);
      setReady(true);
    })();
  ...
  async function addFood(item) {
    ...
    setFoodLog(updated);
    await fbSet("food",selectedDate,{items:updated,date:selectedDate});
    if (selectedDate===getToday()) {
      setWeeklyData(prev=>prev.map(d=>d.date===selectedDate?{...d,kcal:Math.round(updated.reduce((s,f)=>s+(Number(f.kcal)||0),0)),protein:Math.round(updated.reduce((s,f)=>s+(Number(f.protein)||0),0))}:d));
    }
  }
  ...
  async function saveSession(session) { await fbSet("sessions",session.id,session); setSessions(prev=>[...prev,session].sort((a,b)=>a.date.localeCompare(b.date))); }
  async function deleteSession(id) { await fbDel("sessions",id); setSessions(prev=>prev.filter(s=>s.id!==id)); }

  function exportData() {
    const data = { foodLog, suppLog, weightLog, myFoods, sessions, exportedAt:new Date().toISOString() };
  ...
  if (!ready) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"100vh", background:"#1a1a2e" }}>
      <div style={{ fontSize:40, marginBottom:14 }}>gym</div>
      <div style={{ fontSize:14, color:"#fff", fontFamily:"Georgia,serif" }}>Loading your hub...</div>
    </div>
  );

  const TABS = [{id:"home",label:"Home",emoji:"home"},{id:"log",label:"Log",emoji:"log"},{id:"train",label:"Train",emoji:"gym"},{id:"body",label:"Body",emoji:"scale"},{id:"supps",label:"Supps",emoji:"pill"}];
  ...
  return (
    <div style={{ maxWidth:480, margin:"0 auto", minHeight:"100vh", background:"#f8f6f2", fontFamily:"Georgia,serif" }}>
      <div style={{ paddingBottom:74 }}>
        ...
        {tab==="log"&&<LogTab foodLog={foodLog} totals={totals} onAdd={addFood} onRemove={removeFood} myFoods={myFoods} onSaveFood={saveMyFood} onDeleteMyFood={deleteMyFood} selectedDate={selectedDate} onDateChange={loadDayLog}/>}
      ...
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background:"#fff", zIndex:999, borderTop:"1px solid #eee", display:"flex", boxShadow:"0 -4px 24px rgba(0,0,0,0.08)" }}>
        {[{id:"home",label:"Home",e:"🏠"},{id:"log",label:"Log",e:"📝"},{id:"train",label:"Train",e:"🏋️"},{id:"body",label:"Body",e:"⚖️"},{id:"supps",label:"Supps",e:"💊"}].map(t=>(
